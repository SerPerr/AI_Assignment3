        ...
        for _ in range(self.num_simulations):
            ...
            if not done and not truncated:  # If we are not at a terminal state, we can expand
                ...
                if len(untried_actions) > 0:  # If there are still untried actions, we expand by taking one of them
                    ...
                    obs, reward, done, truncated, info = sim_env.step(action)       # Take the action in the simulated environment
                    total_reward += reward                                           # Accumulate the reward obtained during expansion
            ...
            # total_reward is NOT reset - we continue accumulating from where we left off
            depth = 0
...
class InformedMCTSAgent(base.Agent):
    ...
    def act(self, observation, env):
        ...
        for _ in range(self.num_simulations):
            ...
            truncated = False
            total_reward = 0  # ← αρχικοποίηση εδώ, δεν ξαναμηδενίζεται
            ...
            while node.is_fully_expanded(num_actions) and len(node.children) > 0:
                ...
                total_reward += reward  # ← μαζεύουμε reward από selection
            ...
            if not done and not truncated:
                ...
                if len(untried_actions) > 0:
                    # Υπολογίζουμε το prior μία φορά για ΟΛΕΣ τις ενέργειες
                    # και το αποθηκεύουμε στον γονέα - όχι μόνο για την επιλεγμένη
                    if not node.children_prior:
                        ...
                        for a in range(num_actions):
                            node.children_prior[a] = float(prior[a])  # ← αποθηκεύουμε για όλες

                    action = random.choice(untried_actions)
                    obs, reward, done, truncated, info = sim_env.step(action)
                    total_reward += reward  # ← μαζεύουμε reward από expansion
                    new_node = MCTSNode(
                        state=obs,
                        parent=node,
                        action=action,
                        prior=node.children_prior[action]  # ← διαβάζουμε από τον γονέα
                    )
                    node.children[action] = new_node
                    node = new_node

            # --- 3. SIMULATION / ROLLOUT ---
            # total_reward ΔΕΝ μηδενίζεται - συνεχίζουμε από όπου σταματήσαμε
            depth = 0
    ...
    def _select_puct_child(self, node):
        ...
        for action, child in node.children.items():
            exploitation = child.Q / child.N
            exploration = self.c * child.prior * np.sqrt(node.N) / (1 + child.N)
